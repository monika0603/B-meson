...
void EfficiencyPrivateSample()
{
    ...
    TH1D* h_bpfilter_pt_CB = new TH1D("h_bpfilter_pt_CB", "Filter denominator coarse bins", ptBinsSize, &ptBins[0]);
            ...
            if (abs(GenInfo->pdgId[idx])==521) { // B+ find
               ...
               // if (muon1Filter && muon2Filter && bRapFilter) {
                if (bRapFilter) {
                    ...
                    h_bpfilter_pt_CB->Fill(GenInfo->pt[idx_bp]);
                }
                
                if (muon1Acc && muon2Acc && kplusAcc && GenInfo->mass[idx_bp] >=5.18 && GenInfo->mass[idx_bp] <= 5.38){
    ...
    for (int bidx = 0; bidx < BInfo->size; bidx++) {
        ...
        for (int ngen=0; ngen<nMult_gen; ngen++)
        {
          ...
          //  if (dR > 0.011) continue;
           ...
           // if (BInfo->mass[bidx] >=5.18 && BInfo->mass[bidx] <= 5.38 && fabs(v4_b.Rapidity())<2.4){
            if (BInfo->mass[bidx] >=5.18 && BInfo->mass[bidx] <= 5.38){
    ...
    TH1D *h_bp_pt_effPrime_CB = (TH1D*)h_bppt_RL_coarseBins->Clone("h_bp_pt_effPrime_CB");
    h_bp_pt_effPrime_CB->Divide(h_bpfilter_pt_CB);
    binomialError(h_bp_pt_effPrime_CB, h_bpfilter_pt_CB);
    
    for (int i=1; i<=h_bp_pt_effPrime_CB->GetNbinsX(); i++) {
        double a = h_bp_pt_effPrime_CB->GetBinContent(i);
        double b = h_bp_pt_effPrime_CB->GetBinError(i);
        cout<<i<<'\t'<<a<<'\t'<<b<<endl;
    }
